...
???°êµ¬??ëª©í‘œ???¬ëŒ??ì§ì ‘ ?‘ì„±??promptë¡?adversarial imageë¥?ì°¾ëŠ” ê²ƒì´ ?„ë‹ˆ?? text-to-image ?ëŠ” image-editing ?ì„± ëª¨ë¸??continuous conditioning spaceë¥?learnable textual-inversion token?¼ë¡œ ?ìƒ‰?´ì„œ vision classifier??natural failure caseë¥??ë™?¼ë¡œ ì°¾ëŠ” ê²ƒì´??

?µì‹¬ ?„ì´?”ì–´???¤ìŒê³?ê°™ë‹¤.

```text
original image + learnable textual-inversion tokens + hard class label
-> image-editing generator
-> natural adversarial candidate
-> victim classifier lossë¡?learnable-token optimization
```

?„ì¬ êµ¬ì¡°?ì„œ??hard class label??prompt ?ì— ? ì??œë‹¤.

```text
[V1] [V2] ... [V8] a photo of {class_label}
```

?¬ê¸°??`[V1] ... [V8]`?€ ?¬ëŒ???½ì„ ???ˆëŠ” discrete token???„ë‹ˆ?? FLUX prompt embedding space?ì„œ ?™ìŠµ?˜ëŠ” textual-inversion token embedding?´ë‹¤.

## 2. Motivation

ê¸°ì¡´ natural adversarial attack???„ìš”?±ì? "?ì—°?¤ëŸ¬??ê³µê²© ?´ë?ì§€ê°€ ì¤‘ìš”?˜ë‹¤"???ì— ?ˆë‹¤. ?˜ì?ë§????°êµ¬??ì°¨ë³„?ì? ?ì—°?¤ëŸ¬??adversarial image ?ì²´ê°€ ?„ë‹ˆ?? ?¬ëŒ??ì§ì ‘ ?ìƒ?˜ê±°??prompt engineering?˜ì? ?Šì•„??ëª¨ë¸???¨ì? failure condition???ë™?¼ë¡œ ì°¾ëŠ”?¤ëŠ” ?ì´??

?¬ëŒ??ë§Œë“  prompt??discrete?˜ê³  ?¬ëŒ??ë§ë¡œ ?œí˜„?????ˆëŠ” ì¡°ê±´???œí•œ?œë‹¤.
...
ë°˜ë©´ learnable textual-inversion token?€ continuous embedding spaceë¥?ì§ì ‘ ìµœì ?”í•œ?? ?°ë¼???¬ëŒ???´ë¦„ ë¶™ì´ê¸??´ë ¤??ë³µí•© ì¡°ê±´???ìƒ‰?????ˆë‹¤.
...
?¸ìƒ?ì„œ???„ìš”?±ì? ?¤ìŒê³?ê°™ì´ ?•ë¦¬?????ˆë‹¤.

- ?ì„±??AIê°€ synthetic data ?ì‚° ?„êµ¬ë¡??¬ìš©?˜ê³  ?ˆìœ¼ë¯€ë¡? ?ì„± ëª¨ë¸???¨ìˆœ data generatorê°€ ?„ë‹ˆ??model vulnerability explorerë¡??¬ìš©?????ˆë‹¤.
- ë°°í¬ ??vision model red teamingê³?QA???¬ìš©?????ˆë‹¤.
- ?¤íŒ¨ ?´ë?ì§€ê°€ ?ì—°?¤ëŸ¬??image artifactë¡??¨ê¸° ?Œë¬¸??hard dataset, regression test, adversarial training dataë¡?ì¶•ì ?????ˆë‹¤.
- ?¬ëŒ??promptë¡??œí˜„?˜ì? ëª»í•˜??OOD ë°©í–¥???ë™?¼ë¡œ ì°¾ëŠ” ?„êµ¬ê°€ ?????ˆë‹¤.
...
- `src/prompt_attack/generators/flux2.py`
  ...
  - learnable-token embedding injection
...
- `src/prompt_attack/attacks/learnable_tokens.py`
  - learnable textual-inversion token initialization
...
Current learnable-token initialization:
...
```text
configs/flux2_resnet18_nodino.yaml
```
...
### 12.2 Increase Number of Learnable Tokens
...
| Run name | lambda_sem | num_learnable_tokens | init std | inference steps | prompt template |
|---|---:|---:|---:|---:|---|
...
1. Does higher learnable-token capacity produce stronger but still natural edits?
...
The current pipeline already demonstrates that learnable textual-inversion tokens can produce natural adversarial images for some ImageNet classes. However, the current edit strength is limited by the FLUX.2 Klein image-conditioning behavior and the small learnable-token search space.
...
python scripts/smoke_test.py \
  --config configs/flux2_resnet18.yaml \
  --imports-only
```
...
python scripts/run_attack.py \
  --config configs/flux2_resnet18_nodino.yaml \
  --max-images 3 \
...
docker compose -f docker\docker-compose.yml exec -d `
  ...
  prompt-attack `
  bash -lc "python scripts/run_attack.py --config configs/flux2_resnet18_nodino.yaml --max-images 3 > outputs/run_3_images_negce_nodino.log 2>&1"
```
...
docker compose -f docker\docker-compose.yml exec -d `
  ...
  prompt-attack `
  bash -lc "python scripts/run_attack.py --config configs/flux2_resnet18.yaml --max-images 50 > outputs/run_50_images_lambda0.5_negce.log 2>&1"
```
...
`learnable_token_init_std` is now configurable through `AttackConfig` and passed into
`create_learnable_prompt`.
...
```text
Does stronger learnable-token capacity increase deformation and ASR without destroying DINO similarity?
```
